...
RSpec.describe "Api::V1::Recipes", type: :request do
  ...
  describe "happy path" do
    describe "GET /api/v1/recipes" do
      it "returns all publicly visible recipes" do
      ...
      it "returns recipes filtered by drink name" do
        ...
        classic_recipe =
          result.find do |recipe|
            recipe["name"] ==
              "Classic Old Fashioned"
          end

        expect(
          classic_recipe["drink"]["id"]
        ).to eq(@drink.id)

        expect(
          classic_recipe["drink"]["name"]
        ).to eq("Old Fashioned")

        expect(
          classic_recipe["drink"]["username"]
        ).to eq("alice")

        expect(
          classic_recipe["ingredients"].count
        ).to eq(1)

        expect(
          classic_recipe["ingredients"]
            .first["name"]
        ).to eq("Bourbon")
      ...
      it "returns save_count for each recipe" do
        saver = User.create!(
          name: "Charlie",
          username: "charlie",
        ...
        expect(
          recipe_json["save_count"]
        ).to eq(2)
      end

      it "returns created_at for each recipe" do
        log_in(@user)
      ...
      it "sorts recipes by most recent first" do
        older_recipe = Recipe.create!(
          drink: @other_drink,
          name: "Older Margarita",
      ...
      it "sorts recipes by most saved first" do
        ...
        expect(
          result.second["save_count"]
        ).to eq(1)
      end
    end

    describe "GET /api/v1/drinks/:drink_id/recipes" do
      it "returns all publicly visible recipes associated with the drink" do
        Recipe.create!(
          drink: @drink,
          name: "Maple Old Fashioned",
          instructions: "Stir with maple syrup and ice.",
        ...
        expect(
          result.first["drink"]["id"]
        ).to eq(@drink.id)

        expect(
          result.first["drink"]["name"]
        ).to eq("Old Fashioned")

        expect(
          result.first["drink"]["username"]
        ).to eq("alice")
      end

      it "does not return private recipes for the drink" do
        private_recipe = Recipe.create!(
          drink: @drink,
    ...
    describe "GET /api/v1/recipes/:id" do
      it "returns one recipe with its associated drink" do
        ...
        expect(
          result["drink"]["username"]
        ).to eq("alice")
      end

      it "returns save_count" do
        UserRecipe.create!(
          user: @other_user,
          recipe: @recipe
        )
  ...
  describe "sad path" do
    ...
    describe "GET /api/v1/recipes search" do
      ...
      it "falls back safely when the sort option is not recognized" do
        log_in(@user)
